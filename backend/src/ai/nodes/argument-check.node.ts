...
export async function argumentCheckNode(
  ...
  // FIX #1: CARDINALITY CHECK - Force flow switch if multi-select mismatches single-logger flow
  const selectedCount = (state.flowContext.selectedLoggerIds || []).length;
  const singleLoggerFlows = ['health_check', 'financial_report'];

  if (selectedCount > 1 && singleLoggerFlows.includes(flowType)) {
    logger.log(
  ...
  // CRITICAL: Check if already waiting for user input to prevent recursion
  // If we're already prompting for an argument, don't generate a new prompt
  ...
  let updatedContext: FlowContext = { ...state.flowContext };
  const appliedDefaults: string[] = [];

  // Step 1: Apply defaults for optional missing args
  ...
  // Step 2: Find first missing required argument
  for (const spec of specs) {
    if (spec.required && !isArgumentSatisfied(spec, updatedContext)) {
      logger.debug(`Missing required argument: ${spec.name}`);
      ...
      if (spec.type === 'single_logger' || spec.type === 'multiple_loggers') {
        const options = formatLoggerOptions({ loggers: availableLoggers });

        // Handle empty options case - no loggers available
        if (options.length === 0) {
          logger.debug(
...
Once you have data, I can help with financial reports, health checks, performance comparisons, and more.`,
        ...
        selectionArgs = createEnhancedSelectionArgs({
          prompt,
          contextMessage,
          options,
          selectionType:
            spec.type === 'multiple_loggers' ? 'multiple' : 'single',
          inputType: 'dropdown',
          preSelectedValues,
          // Pass min/max count for multi-select validation
          minCount:
            spec.type === 'multiple_loggers' ? spec.minCount : undefined,
          maxCount:
            spec.type === 'multiple_loggers' ? spec.maxCount : undefined,
          flowHint: {
      ...
      // Build the AI message with context-aware content
      const messageContent = contextMessage
        ? `${contextMessage}\n\n${prompt}`
        : prompt;

      // Encode flow context as hidden metadata for persistence across invocations
      // This allows the router to restore context when user responds to selection
      ...
      return {
        ...
        flowContext: {
          ...
          currentPromptArg: spec.name,
          argumentSpec: specs,
          waitingForUserInput: true, // CRITICAL: Prevent re-prompting on next turn
        },
        pendingUiActions: [
          {
            toolCallId,
